...
class ViewController: UIViewController {
    ...
    private lazy var classificationRequest: VNCoreMLRequest = {
        do {
            ...
            // 注意点：替换自己的Model
            let model = try VNCoreMLModel(for: CarRecognition(configuration: config).model)
    ...
    func handle(_ results: [VNObservation]?) {
        ...
        if let obs = obs {
            DispatchQueue.main.async {
                let obs = obs.sorted { ob1, ob2 in
                    ob1.confidence > ob2.confidence
                }.prefix(3)

                print(obs.count)

                for ob in obs {
                    self.str += "\(ob.identifier), \(ob.confidence)\n"
                }

                self.carInfo.text = self.str
            }
        }
    }
}
